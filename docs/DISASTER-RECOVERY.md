...
2. Docker image **digest** (e.g. `hillstreet/open-system@sha256:…`)
...
5. Run the container with `gateway run` and the same env **names** as production (values from secret store)
6. Mount `/opt/data`
